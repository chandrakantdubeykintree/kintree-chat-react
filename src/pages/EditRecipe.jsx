...
import { useEditRecipe, useRecipe } from "@/hooks/useRecipes";
...
export default function EditRecipe() {
  ...
  useEffect(() => {
    if (recipe) {
      const recipeData = recipe.post_data;
      ...
      form.reset({
        ...
        attachment_id: recipeData.attachment?.id || null,
        attachment_id_url: recipeData.attachment?.url || null,
        duration: recipeData.duration / 60, // Convert seconds to minutes
        recipe_category_id: recipeData.category.id.toString(),
        privacy: recipe.privacy,
        ingredients: recipeData.ingredients?.map((ing) => ({
          text: ing.name,
          id: ing.id.toString(), // Add id if needed for tracking
        })) || [{ text: "" }],
        steps: recipeData.steps?.map((step) => ({
          description: step.description,
          attachment_id: step.attachment?.id || null,
          attachment_url: step.attachment?.url || null,
          id: step.id.toString(), // Add id if needed for tracking
        })) || [{ description: "", attachment_id: null }],
      });

      // Set privacy dropdown
      setSelectedPrivacy(PRIVACYDROPDOWN.find((p) => p.id === recipe.privacy));
    }
  }, [recipe]);
  ...
  const {
    ...
    name: "ingredients",
    keyName: "fieldId", // Add this to avoid key conflicts
  });
  ...
  // Watch for changes in both ingredients and steps
  const watchIngredients = form.watch("ingredients");
  const watchSteps = form.watch("steps");

  // Effects to handle automatic removal of empty fields
  useEffect(() => {
    const currentIngredients = form.getValues("ingredients");
  ...
  // Rest of the JSX structure same as CreateRecipe
  return (
    <AsyncComponent>
            ...
            <Form {...form}>
              ...
              >
                <div className="flex justify-between">
                  <h2 className="text-2xl font-bold">{t("edit_recipe")}</h2>
                  <FormField
                    control={form.control}
                    name="privacy"
                    render={({ field }) => (
                        ...
                        <FormControl>
                          <div className="space-y-2">
                            <div className="relative w-full">
                              ...
                              <div className="absolute w-full flex justify-between -ml-[1px]">
                                {Array.from(
                                  { length: 3 },
                                  (_, i) => i * 90
                                ).map((value) => (
